		...
		<div class="metronome-container q-mx-auto" style="max-width: 500px">
			<q-card>
				...
				<q-card-section class="text-center">
					...
					<q-slider v-model="bpm" :min="30" :max="300" :step="1" label color="primary" class="q-mb-lg" />

					<div class="row justify-center q-gutter-sm q-mb-lg">
						<q-input v-model.number="bpm" type="number" filled dense style="width: 100px" :min="30" :max="300" label="BPM" />
						<q-input v-model.number="beatsPerMeasure" type="number" filled dense style="width: 100px" :min="1" :max="12" label="Beats" />
					</div>
					...
					<q-btn :icon="isPlaying ? 'stop' : 'play_arrow'" :label="isPlaying ? 'Stop' : 'Start'" :color="isPlaying ? 'negative' : 'positive'" size="xl" rounded @click="toggle" />
				</q-card-section>
			</q-card>
		</div>
	</q-page>
